...
public abstract class BaseUI implements IBoardUI {
	 ...
	 * 객체마다 상속받았어 이제 이 부모객체가 상속받은 인터페이스 쓸라해 인터페이스에서 execute로 정의 했어 사용해 근데 물려받은 자식객체는
	 * 오버라이딩을 해서 써야되 부모가 물려받은 인터페이스를 쓸라해 오버라이딩 해 오류나... 아 인터페이스엔 구현된게 없지 이름만 지어논
	 * 곳이니깐.. 그래 이건 추상 클래스로 만들어 버리자 그럼 execute 사용할 수 있다
	 */
	...
	protected String scanStr(String msg) {
		/**
		 * 이 기능은 상속받은 객체만 사용하게 할거니깐 protected로 바꿔야 겠당 상속받은 객체가 호출할 경우 해당 값 가져온다
		 * protected 같은 패키지 내에서만 접근 가능 단 다른 패키지의 상속관계인 경우 상속받은 자식 클래스 내부에서만 부모객체 접근이
		 * 가능하다 다른 패키지에서 부모객체를 생성해서 사용하는 건 불가능하지만 같은 패키지는 가능하다
		 */
